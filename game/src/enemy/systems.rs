...
pub fn spawn(
    ...
    for (texture, transform, dim, frames) in [
        ("john.png", Transform::from_xyz(10., 0.1, 3.), 64, 3),
        ("kai.png", Transform::from_xyz(8., 0.1, 4.), 128, 4),
    ] {
        let texture = asset_server.load(texture);

        let spritesheet = Spritesheet::new(frames, 1);
